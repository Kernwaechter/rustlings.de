...
// Wir benutzen diesen Fehlertyp für die `TryFrom`-Umwandlungen.
#[derive(Debug, PartialEq)]
enum IntoColorError {
    // Falsche Länge des Slice
    BadLen,
    // Fehler bei der Ganzzahlumwandlung
    IntConversion,
}

// TODO: Tupel-Implementierung.
// Korrekte RGB-Farbwerte müssen Ganzzahlen im Bereich 0..=255 sein.
...
// TODO: Slice-Implementierung.
// Diese Implementierung muss die Länge des Slice prüfen.
...
fn main() {
    // Benutzung der Funktion `try_from`.
    let c1 = Color::try_from((183, 65, 14));
    println!("{c1:?}");

    // Da `TryFrom` für `Color` implementiert ist, können wir `TryInto`
    // benutzen.
    let c2: Result<Color, _> = [183, 65, 14].try_into();
    println!("{c2:?}");

    let v = vec![183, 65, 14];
    // Bei einem Slice sollten wir die Funktion `try_from` benutzen
    let c3 = Color::try_from(&v[..]);
    println!("{c3:?}");
    // oder den Slice in runde Klammern setzen und `try_into` benutzen.
