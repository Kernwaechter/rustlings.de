...
// Entspricht `count_collection_iterator` und `count_iterator`, iteriert
// aber so, als wäre die Collection ein einzelner Container statt eines
// Containers von Containern (genauer: ein einzelner Iterator statt eines
// Iterators von Iteratoren).
fn count_collection_iterator_flat(
    ...
    collection
        .iter()
        .flat_map(HashMap::values) // oder einfach `.flatten()`, wenn man den Standard-Iterator (`HashMap::iter`) möchte
